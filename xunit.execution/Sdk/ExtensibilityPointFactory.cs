...
namespace Xunit.Sdk
{
	...
	public static class ExtensibilityPointFactory
	{
		...
		{
			if (!typeof(IXunitTestCollectionFactory).IsAssignableFromEx(result))
				return false;

			return result.GetConstructorsEx()
													 .Any(ctor =>
																{
																	...
																	return testAssemblyTypeInfo.IsAssignableFromEx(parameters[0].ParameterType);
																});
		}
	}
}
