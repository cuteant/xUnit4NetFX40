...
namespace Xunit
{
	...
	public abstract class MemberDataAttributeBase : DataAttribute
	{
		...
		Func<object> GetFieldAccessor(Type type)
		{
			...
			{
				fieldInfo = reflectionType.GetFieldEx(MemberName, false);
				if (fieldInfo != null)
					break;
			}
#endif
		...
		Func<object> GetMethodAccessor(Type type)
		{
			...
			for (var reflectionType = type; reflectionType != null; reflectionType = reflectionType.BaseType)
			{
				methodInfo = reflectionType.GetMethod(MemberName, parameterTypes);
				if (methodInfo != null)
					break;
			}
#endif
		...
		Func<object> GetPropertyAccessor(Type type)
		{
			...
			for (var reflectionType = type; reflectionType != null; reflectionType = reflectionType.BaseType)
			{
				propInfo = reflectionType.GetPropertyEx(MemberName, false);
				if (propInfo != null)
					break;
			}

			if (propInfo == null || propInfo.GetGetMethod(true) == null || !propInfo.GetGetMethod(true).IsStatic)
				return null;
#endif
